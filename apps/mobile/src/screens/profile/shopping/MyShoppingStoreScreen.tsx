...
import { View, Text, ScrollView, TouchableOpacity, Switch, ActivityIndicator, RefreshControl } from 'react-native';
...
export default function MyShoppingStoreScreen({ navigation }: any) {
  ...
  const load = useCallback(async () => {
    try {
      const res = await fetchApi('/shopping/my-store');
  ...
  const toggleOpen = async () => {
    ...
    setToggling(true);
    try {
      await fetchApi(`/shopping/stores/${store.id}`, {
        method: 'PUT',
  ...
  const styles = useThemedStyles(({ COLORS, TYPOGRAPHY, SPACING }) => ({
    ...
    productPrice: { color: '#8B5CF6', fontWeight: '700', fontSize: 14 },
    ...
    createBtn: { backgroundColor: '#8B5CF6', paddingHorizontal: 24, paddingVertical: 12, borderRadius: 24 },
  ...
  if (!store) return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.empty}>
        ...
        <TouchableOpacity style={styles.createBtn} onPress={() => navigation.navigate('AddEditShoppingStore', {})}>
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <View style={styles.header}>
        ...
        <TouchableOpacity onPress={() => navigation.navigate('AddEditShoppingStore', { store })}>
      ...
      >
        <LinearGradient colors={GRADIENTS.primary} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.storeCard}>
        ...
        <View style={styles.actions}>
          <TouchableOpacity style={styles.actionBtn} onPress={() => navigation.navigate('AddEditShoppingProduct', { storeId: store.id })}>
            <Ionicons name="add-circle" size={20} color="#8B5CF6" />
            <Text style={styles.actionText}>Add Product</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.actionBtn} onPress={() => navigation.navigate('ShoppingStoreOrders')}>
        ...
        {(store.products || []).length === 0 ? (
          <View style={styles.emptyProducts}>
            ...
            <TouchableOpacity onPress={() => navigation.navigate('AddEditShoppingProduct', { storeId: store.id })}>
              <Text style={{ color: '#8B5CF6', fontWeight: '600', fontSize: 13 }}>Add your first product</Text>
            </TouchableOpacity>
          </View>
        ) : (
                ...
                <View key={p.id} style={styles.productRow}>
                  ...
                  <View style={styles.productActions}>
                    <TouchableOpacity
                      ...
                      onPress={() => navigation.navigate('AddEditShoppingProduct', { storeId: store.id, product: p })}
                    >
