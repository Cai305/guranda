...
export default function AddEditStoreScreen({ navigation, route }: any) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const res = isEdit
        ? await fetchApi(`/eat/stores/${existing.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
        : await fetchApi('/eat/stores', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
      if (!res.ok) {
