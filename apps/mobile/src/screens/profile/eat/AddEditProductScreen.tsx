...
const PRODUCT_CATS = ['Starters', 'Mains', 'Sides', 'Drinks', 'Desserts', 'Specials', 'Other'];
...
export default function AddEditProductScreen({ navigation, route }: any) {
  ...
  const styles = useThemedStyles(({ COLORS, TYPOGRAPHY, SPACING }) => ({
    ...
    saveBtn: { backgroundColor: '#ef4444', borderRadius: 14, padding: 16, alignItems: 'center' },
  ...
  const save = async () => {
    ...
    try {
      ...
      const res = isEdit
        ? await fetchApi(`/eat/stores/${storeId}/products/${existing.id}`, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) })
        : await fetchApi(`/eat/stores/${storeId}/products`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
      if (!res.ok) {
