...
import { BlueprintDto } from '@mxit2/types';

type BrowseBlueprintDto = BlueprintDto & { createdByUser?: { id: string; username: string } };

// Blueprint Marketplace (Phase 4) — browse real PUBLISHED Blueprints from
// GET /blueprints/browse. Tapping a card goes to BlueprintDetailScreen,
// which owns the real purchase/review/run flow — this screen is read-only.
// Exact structural twin of FeatureMarketplaceScreen; the top segmented
// control is the real, reachable path between the two marketplaces (both
// screens carry it) so neither is orphaned off the single "Browse
// Marketplace" entry point in ProfileScreen.
export default function BlueprintMarketplaceScreen({ navigation }: any) {
  const { theme } = useTheme();
  const { COLORS, TYPOGRAPHY } = theme;

  const [search, setSearch] = useState('');
  const [blueprints, setBlueprints] = useState<BrowseBlueprintDto[]>([]);
  ...
  const styles = useThemedStyles(({ COLORS, RADIUS, SPACING }) => ({
    ...
    cardCreator: { color: COLORS.textMuted, fontSize: 11, fontWeight: '700' },
  ...
  const load = useCallback(async (query: string) => {
    ...
    try {
      ...
      const res = await fetchApi(`/blueprints/browse?${params.toString()}`);
      const data = await res.json();
      if (!res.ok) throw new Error(data?.message || 'Could not load the Marketplace.');
      setBlueprints(data as BrowseBlueprintDto[]);
    } catch (e: any) {
  ...
  return (
    <SafeAreaView style={styles.root} edges={['top']}>
      ...
      <View style={styles.segmentWrap}>
        <TouchableOpacity style={styles.segmentBtn} onPress={() => navigation.replace('FeatureMarketplace')}>
          <Text style={styles.segmentText}>Features</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.segmentBtn, styles.segmentBtnActive]}>
          <Text style={[styles.segmentText, styles.segmentTextActive]}>Blueprints</Text>
        </TouchableOpacity>
      </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptyTitle}>No published Blueprints yet</Text>
              <Text style={styles.emptyBody}>Publish a Blueprint from the Blueprint Builder to see it here.</Text>
            </View>
          }
          renderItem={({ item }) => (
            <TouchableOpacity
              style={styles.card}
              activeOpacity={0.85}
              onPress={() => navigation.navigate('BlueprintDetail', { blueprintId: item.id })}
            >
              ...
              <View style={styles.cardBody}>
                ...
                {item.createdByUser && (
                  <Text style={styles.cardCreator}>by @{item.createdByUser.username}</Text>
                )}
